	...
	$(btnObj).children().click(function(){
		...
		let template = [
			'<div class="input-field col s12 optDiv">',
				...
				$('<input type="text" class="validate">').attr("id", tempID).prop('outerHTML'),
			...
			'<div class="card q_card">',
				'<div class="row">',
					'<div class="input-field col s12">',
						$('<input type="text" class="validate">').attr("id", "title"+i).prop('outerHTML'),
						$('<label>Question</label>').attr("for", "title"+i).prop('outerHTML'),
					'</div>',
					'<div class="input-field col s12 optDiv">',
						$('<i class="material-icons prefix">check_box_outline_blank</i>').prop('outerHTML'),
						$('<input type="text" class="validate">').attr("id", i+"ans0").prop('outerHTML'),
					...
					'<div class="input-field col s12 l6">',
						$('<input type="text" class="validate">').attr("id", "min"+i).prop('outerHTML'),
						$('<label>Minimium no. of choice(s)</label>').attr("for", "min"+i).prop('outerHTML'),
					'</div>',
					'<div class="input-field col s12 l6">',
						$('<input type="text" class="validate">').attr("id", "max"+i).prop('outerHTML'),
