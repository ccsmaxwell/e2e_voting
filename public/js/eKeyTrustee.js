...
$("#btn_submit").click(function(){
	...
	var e = bigInt(forge.md.sha256.create().update(msg).digest().toHex(), 16);

	var f_b64 = hexToBase64(s.add(e.multiply(x)).mod(p.minus(1)).toString(16));
	var a_b64 = hexToBase64(a.toString(16))
	...
	var curr_x = bigInt(base64ToHex($("#curr_pri").val()), 16);

	var m = bigInt(forge.md.sha256.create().update(JSON.stringify(data)).digest().toHex(), 16);
	...
	var s1 = g.modPow(r,p);
	var s2 = m.minus(curr_x.multiply(s1).mod(p.minus(1))).add(p.minus(1)).multiply(r.modInv(p.minus(1))).mod(p.minus(1));
